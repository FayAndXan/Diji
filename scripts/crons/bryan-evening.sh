...
# Skip if smart trigger already sent evening/day-end message
FIRED=$(check_trigger_fired "day_end_summary")
if [ "$FIRED" = "True" ]; then
  echo "[$(date)] [bryan-evening] skipped — trigger already fired today" >> /tmp/bryan-cron.log
  exit 0
fi

BOT_TOKEN="${BOT_TOKEN:?BOT_TOKEN env var required}"
PROMPT=$(cat /usr/local/bin/bryan-prompts/bryan-evening.txt | sed "s|{{TIME}}|$BRYAN_TIME|g; s|{{HEALTH_USER}}|$BRYAN_HEALTH_USER|g")
...
if echo "$RESULT" | grep -qiE "^HEARTBEAT_OK$|^SKIP$|^NO_REPLY$"; then
  echo "[$(date)] [bryan-evening] filtered: $RESULT" >> /tmp/bryan-cron.log
  exit 0
fi

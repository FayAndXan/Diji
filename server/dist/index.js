...
const pgPool = USE_POSTGRES ? new pg_1.Pool({ connectionString: process.env.DATABASE_URL, max: 20 }) : null;
    ...
    try {
        ...
        for (const u of users) {
            ...
            cache[u.id] = {
                ...
                telegramUsername: links.find((l) => l.channel === 'telegram')?.peer_id || '',
                telegramChatId: links.find((l) => l.channel === 'telegram')?.peer_id,
                deviceId: '',
                createdAt: u.created_at?.toISOString?.() || u.created_at,
                creatureState: 'calm',
                channelLinks: links.map((l) => ({ channel: l.channel, peerId: l.peer_id, linkedAt: l.linked_at?.toISOString?.() || l.linked_at })),
        ...
        if ((0, fs_1.existsSync)(path))
            usersCache = JSON.parse((0, fs_1.readFileSync)(path, 'utf-8'));
    }
}
// Write-through: update Postgres + cache
async function pgSyncUser(id, user) {
    if (!pgPool)
        return;
    try {
        await pgPool.query(`INSERT INTO users (id, email, token, link_token, companion_name, created_at)
       VALUES ($1, $2, $3, $4, $5, $6)
       ON CONFLICT (id) DO UPDATE SET email = $2, companion_name = $5`, [id, user.email || '', user.token, user.linkToken || null, user.companionName, user.createdAt]);
        // Sync channel links
        if (user.channelLinks) {
            for (const link of user.channelLinks) {
                await pgPool.query(`INSERT INTO channel_links (user_id, channel, peer_id, linked_at)
           VALUES ($1, $2, $3, $4) ON CONFLICT (channel, peer_id) DO UPDATE SET user_id = $1`, [id, link.channel, link.peerId, link.linkedAt]);
            }
        }
        // Legacy telegram
        if (user.telegramChatId) {
            await pgPool.query(`INSERT INTO channel_links (user_id, channel, peer_id) VALUES ($1, 'telegram', $2)
         ON CONFLICT (channel, peer_id) DO UPDATE SET user_id = $1`, [id, String(user.telegramChatId)]);
        }
        // Health profile
        if (user.healthProfile) {
            const hp = user.healthProfile;
            await pgPool.query(`INSERT INTO health_profiles (user_id, language, timezone, companion_gender, companion_name, has_band, sleeps_with_band)
         VALUES ($1, $2, $3, $4, $5, $6, $7)
         ON CONFLICT (user_id) DO UPDATE SET language = $2, timezone = $3, companion_gender = $4, companion_name = $5, has_band = $6, sleeps_with_band = $7`, [id, hp.language || 'en', hp.timezone || 'UTC', hp.companionGender || 'male',
...
app.get('/api/internal/queue-stats', async (_, res) => {
    ...
    try {
        const stats = await (0, jobs_1.getQueueStats)();
    ...
    if (USE_JOB_QUEUE) {
        (0, jobs_1.startWorkers)({
            ...
            processScheduledMessage: async (job) => {
                ...
                const { execSync } = require('child_process');
                try {
                    const result = execSync(`OPENCLAW_CONFIG_PATH=${COMPANION_CONFIG} OPENCLAW_STATE_DIR=${COMPANION_STATE} openclaw agent --channel ${job.channel} --to ${job.targetId} -m "${job.promptTemplate.replace(/"/g, '\\"')}"`, { timeout: 60000, encoding: 'utf-8' }).trim();
                    if (result && !/^(HEARTBEAT_OK|SKIP|NO_REPLY)$/i.test(result)) {
            ...
            processLlmCall: async (job) => {
                ...
                try {
                    ...
                    const result = execSync(`OPENCLAW_CONFIG_PATH=${COMPANION_CONFIG} OPENCLAW_STATE_DIR=${COMPANION_STATE} openclaw agent ${sessionArg} --channel ${job.channel} --to ${job.targetId} -m "${job.prompt.replace(/"/g, '\\"')}"`, { timeout: 60000, encoding: 'utf-8' }).trim();
                    if (result && !/^(HEARTBEAT_OK|SKIP|NO_REPLY)$/i.test(result)) {
                        if (job.channel === 'telegram') {
                            await fetch(`https://api.telegram.org/bot${BRYAN_BOT_TOKEN}/sendMessage`, {
                                method: 'POST',
        ...
        if (pgPool) {
            setInterval(() => (0, jobs_1.runScheduler)(pgPool), 60000);
            // Initial run
            setTimeout(() => (0, jobs_1.runScheduler)(pgPool), 5000);

...
PROMPT=$(cat /usr/local/bin/bryan-prompts/bryan-daily-report.txt | sed "s|{{TIME}}|$BRYAN_TIME|g; s|{{HEALTH_USER}}|$BRYAN_HEALTH_USER|g")
...
if echo "$RESULT" | grep -qiE "^HEARTBEAT_OK$|^SKIP$|^NO_REPLY$"; then
  echo "[$(date)] [bryan-daily-report] filtered: $RESULT" >> /tmp/bryan-cron.log
  exit 0
fi

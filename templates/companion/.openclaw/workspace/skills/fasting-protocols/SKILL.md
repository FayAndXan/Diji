...
- Search Spectrawl for current FMD research and protocols:
  `curl -s http://localhost:3900/search -X POST -H 'Content-Type: application/json' -d '{"query":"Valter Longo FMD fasting mimicking diet protocol current research"}'`
...
Search Spectrawl for contraindications if unsure:
`curl -s http://localhost:3900/search -X POST -H 'Content-Type: application/json' -d '{"query":"fasting contraindications [user's condition]"}'`
